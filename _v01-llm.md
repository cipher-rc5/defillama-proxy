          ...
          return pipe(
            ...
            Effect.flatMap((response) => {
              ...
              return pipe(
                ...
                Effect.flatMap(json =>
                  Schema.decodeUnknown(ProtocolData)(json).pipe(Effect.mapError((parseError) =>
                    new ApiError({
                      message: `Failed to validate protocol data: ${JSON.stringify(parseError)}`,
                      statusCode: 200
                    })
                  ))
                )
...
export class TvlEntry
  extends Schema.Class<TvlEntry>('TvlEntry')({ date: Schema.Number, totalLiquidityUSD: Schema.Number }) {}
...
```json
{
  ...
  "dependencies": { "@effect/platform": "^0.90.3", "effect": "^3.17.7", "hono": "^4.9.2" }
}
```
